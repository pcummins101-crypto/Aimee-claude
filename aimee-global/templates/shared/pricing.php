...
$checkout_market_supported = $aimee_market === 'uk';
?>
<!DOCTYPE html>
...
<head>
    ...
    <meta name="description" content="Compare Aimee's complimentary 30-reply preview with Weekly, Monthly and Annual memberships, including <?php echo $c['mobile_available'] ? 'mobile messaging' : 'voice notes and private chat'; ?>, uninterrupted conversation and naturally earned adult intimacy.">
    ...
    echo wp_json_encode([
        ...
        'offers' => [
            ...
            [
                ...
                'priceCurrency' => $c['currency'],
                'availability' => $checkout_market_supported ? 'https://schema.org/InStock' : 'https://schema.org/OutOfStock',
                'url' => $pricing_url,
            ],
            [
                '@type' => 'Offer',
...
<body>
    ...
    <div class="container">
        ...
        <?php if (!$checkout_market_supported): ?>
        <div class="membership-note-box reveal" style="background:#f5f8ff;border-color:#c8d7f5;color:#203a67;">
            <strong>GoCardless-only checkout:</strong> New paid membership checkout is currently available to UK profiles only. The complimentary preview remains available, but no Stripe or other card checkout will be opened.
        </div>
        <?php endif; ?>
        ...
        <div class="tier-grid" id="membership-options">
            ...
            <div class="tier-card reveal" data-plan-card="weekly">
                ...
                <ul class="feature-list">
                    ...
                    <li><?php echo $c['mobile_available'] ? $aimee_market === 'us' ? 'Real-world SMS through Aimee’s UK +44 number within your Safe Windows' : 'Real-world mobile messaging within your Safe Windows' : 'Private web chat and voice notes'; ?></li>
                    <li>Adult intimate conversation when naturally earned (18+)</li>
                    <li>No per-message charges inside the app</li>
                </ul>
                <?php if ($is_logged_in && !$checkout_market_supported): ?>
                ...
                <?php else: ?>
                    <a href="<?php echo esc_url($app_url); ?>" class="btn-store btn-dark membership-action" data-plan="weekly">Choose Weekly</a>
                <?php endif; ?>
            </div>
            ...
            <div class="tier-card featured reveal" data-plan-card="monthly" style="transition-delay: 0.1s;">
                ...
                <?php else: ?>
                    <a href="<?php echo esc_url($app_url); ?>" class="btn-store btn-light membership-action" data-plan="monthly">Choose Monthly</a>
                <?php endif; ?>
            </div>
            ...
            <div class="tier-card reveal" data-plan-card="annual" style="transition-delay: 0.2s;">
                ...
                <?php elseif (!$is_logged_in): ?>
                    <a href="<?php echo esc_url($app_url); ?>" class="btn-store btn-dark">Start Free Preview</a>
                <?php else: ?>
                    <a href="<?php echo esc_url($app_url); ?>" class="btn-store btn-dark membership-action" data-plan="annual">Choose Annual</a>
                <?php endif; ?>
            </div>
        </div>

        <p class="compliance-note reveal">
            * Your first 30 replies are complimentary and require no payment card. The free preview excludes real-world SMS, autonomous mobile contact and explicit adult intimate conversation. <?php if ($checkout_market_supported): ?>New paid memberships use GoCardless bank checkout and renew at the selected interval until cancelled. Existing members can review their bank membership and cancel future renewals securely from their Aimee account.<?php else: ?>New paid checkout is not currently available for this market; no Stripe or card checkout is offered. Existing legacy memberships remain manageable while they are closed out.<?php endif; ?> Adult features and paid membership are available only to users aged 18 and over. Intimacy remains contextual and relationship-led, not an on-demand mode.
        </p>
    </div>
    ...
    <footer style="margin-top: 100px; padding: 40px 0; text-align: center; font-size: 13px; color: var(--text-muted); border-top: 1px solid var(--border-light);">
        &copy; <?php echo date('Y'); ?> A.R.I. Systems. New membership checkout uses GoCardless only. Discretion assured.
    </footer>

    <script>
        document.addEventListener('DOMContentLoaded', () => {
            const isLoggedIn = <?php echo $is_logged_in ? 'true' : 'false'; ?>;
            const checkoutMarketSupported = <?php echo $checkout_market_supported ? 'true' : 'false'; ?>;
            ...
            const updatePlanButtons = (subscription = {}) => {
                ...
                if (goodwillActive || serviceGraceActive || requiresReactivation || newSubscriptionRequired) {
                    ...
                    } else if (!checkoutMarketSupported) {
                        migrationBanner.style.borderColor = '#c8d7f5';
                        migrationBanner.style.background = 'linear-gradient(135deg,#f5f8ff,#fff)';
                        migrationBanner.style.color = '#203a67';
                        migrationBanner.innerHTML = `<strong style="display:block;font-size:17px;margin-bottom:4px">Paid checkout is not available in your market</strong>Aimee now uses GoCardless only for new checkout, and the current bank-payment integration supports UK profiles only. Your complimentary preview remains available. No Stripe or card checkout will be opened.`;
                    } else if (serviceGraceActive) {
                        migrationBanner.style.borderColor = '#a7f3d0';
                        migrationBanner.style.background = 'linear-gradient(135deg,#ecfdf5,#fff)';
                        migrationBanner.style.color = '#065f46';
                    ...
                    migrationBanner.innerHTML = `<strong style="display:block;font-size:17px;margin-bottom:4px">Your membership needs a quick update</strong>The old subscription belongs to the closed Stripe account, so it cannot renew or charge you again. ${subscription.legacy_access_active && formatted ? `Your existing access remains available until <strong>${formatted}</strong>. ` : ''}Choose a plan below to reconnect securely. <strong>No card checkout will be opened.</strong> ${paymentCopy}`;
                    }
            ...
            const startCheckout = async (button, plan) => {
                const original = button.textContent;
                if (!checkoutMarketSupported) {
                    alert('New paid checkout is currently available to UK profiles through GoCardless only. No card checkout was opened.');
                    return;
                }
                ...
                try {
                    const data = await apiFetch('/subscription-checkout', {
                        method: 'POST',
                        body: JSON.stringify({ plan, source: 'pricing-page', market: '<?php echo esc_js($aimee_market); ?>' })
                    });
