...
function streamTurn(payload,bubble){var live=false;function ensureLive(){if(live)return;live=true;bubble.classList.add('live');setBubbleText(bubble,'')}
 return fetch(cfg.stream,{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/json','X-WP-Nonce':cfg.nonce,'Accept':'text/event-stream'},body:JSON.stringify(payload)}).then(function(r){
  ...
  function handle(ev,data){if(ev==='delta'){ensureLive();current+=data.text||'';setBubbleText(bubble,current);scrollBottom(false)}else if(ev==='replace'){ensureLive();current=data.text||'';setBubbleText(bubble,current)}else if(ev==='status'){setStatus({photo:'choosing a photo…',writing:'typing…',searching:'looking that up…',reading:'reading…'}[data.state]||'thinking…',true)}else if(ev==='done'){done=data}else if(ev==='error'){failed=data}}
